...
export default function FilterHome() {
  ...
  const { setPokemonFilter, filterInput } = useStore((state) => state.pokemon);
  const { resetSliceAmount, setFilteredPage } = useStore(
    (state) => state.pagination
  );
  const userFilterInput = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (userFilterInput?.current) userFilterInput.current.value = filterInput;
  ...
  const handleClearFilter = () => {
    ...
    resetSliceAmount();
    setFilteredPage(1);
    if (userFilterInput?.current) userFilterInput.current.value = "";
  };
      ...
      <div className="mt-1 flex rounded-md shadow-md ">
        <div className="relative flex items-stretch flex-grow focus-within:z-10">
          ...
          <input
            ...
            onChange={() => handleFilterChange(userFilterInput?.current?.value)}
          />
        </div>
        <button
